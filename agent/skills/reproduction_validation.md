...
`classify_issue` plans `needs reproduction` and returns the fact `REPRODUCTION_REQUEST`. The templated request is appended by `apply_triage`, with the guide and starter templates the repository configured. Write at most one sentence before it, for example a thank you for the report.
...
- `usesPackage`: the project depends on the repository's package. `null` means it could not be read, which is fine for StackBlitz projects and CodeSandbox devboxes.
